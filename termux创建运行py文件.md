...
4. 进入新目录在 Termux 中使用 cd 命令进入新目录：

  
5.   cd /storage/emulated/0/termux_files

  
7. 验证当前目录使用 pwd 命令验证你是否已经进入正确的目录：
8. pwd你应该会看到输出：/storage/emulated/0/termux_files

9. 列出目录内容使用 ls 命令列出目录中的文件和文件夹：ls你应该会看到 hello.py 文件。

10. 运行 Python 文件在 Termux 中运行 Python 文件：python hello.py你应该会看到输出：你好，
